...
flag_connected = 0 # Indica que estamos conectados por SSH a la Raspberry
###########################################
...
def move_delta (modo, dir):
    if modo == 0:
        ...
        comando = submit_target (modo, new_x0, new_x1, new_x2)
        return comando
...
def submit_target (modo, x0, x1, x2):
    ...
    return string_comando

def go_home ():
    global flag_tipo_input
    flag_tipo_input = 1
    comando = submit_target(flag_tipo_input, 45, 90, 90)
    return comando
...
def get_login():
    ...
    layout = [[sg.Text("Log In", size =(15, 1), font=40)],
            [sg.Text("Host", size =(15, 1), font=16),sg.InputText(default_text = '192.168.0.11' ,key='-host-', font=16, size =(15,1))],
            [sg.Text("Username", size =(15, 1), font=16),sg.InputText(default_text = 'pi' ,key='-usrnm-', font=16, size =(15,1))],
            [sg.Text("Password", size =(15, 1), font=16),sg.InputText(key='-pwd-', password_char='*', font=16, size =(15,1))],
            [sg.Button('Ok'),sg.Button('Cancel')]]

    window = sg.Window("Conectar a Raspberry", layout)
...
colInput = [[sg.Text("", key='in_x0_title', size=(3,1)), sg.In(default_text='0', key='in_x0', size=(10,1)), sg.Text("", key='mm_deg0') ],

            [sg.Text("", key='in_x1_title', size=(3,1)), sg.In(default_text='0', key='in_x1', size=(10,1)), sg.Text("", key='mm_deg1') ],

            [sg.Text("", key='in_x2_title', size=(3,1)), sg.In(default_text='0', key='in_x2', size=(10,1)), sg.Text("", key='mm_deg2') ]]

connecting_layout = [[sg.Text("Conectando a Raspberry...")]]
...
while True:
    ...
    if event in ("Left:37", "a","A"):
        print("izq")
        comdelta = move_delta(flag_tipo_input, "-x0")
        if flag_connected == 1:
            try:
    ...
    if event in ("Up:38", "w","W"):
        print("adelante")
        comdelta = move_delta(flag_tipo_input, "+x1")
        if flag_connected == 1:
            try:
                stdin, stdout, stderr = client.exec_command(comdelta)
    ...
    if event in ("q","Q"):
        print("der")
        comdelta = move_delta(flag_tipo_input, "-x2")
        if flag_connected == 1:
            try:
                stdin, stdout, stderr = client.exec_command(comdelta)
    ...
    if event == "Submit":
        ...
        comsubmit = submit_target(flag_tipo_input, input0, input1, input2)
        print("-------------------------")
    ...
    if event =="Connect":
        ...
        try:
            ...
            HOST, USER, PWD = get_login()
            client = paramiko.SSHClient()
            client.set_missing_host_key_policy( paramiko.AutoAddPolicy())

            #connect_pop = sg.Window("Conectando", connecting_layout)

            client.connect(HOST, username = USER, password = PWD)

            #### Comandos para entrar al script de Python ####

            lista_inicializacion = ['ls']
